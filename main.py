...
def parse_arg():
    ...
    parser.add_argument('--load_params', type=str, default='models/pcnn_lr_0.00020_nr-resnet5_nr-filters160_0.pth',
...
def train(config):
    ...
    trainset, train_loader, testset, test_loader, classes = load_data(batch_size=config.batch_size)
    obs = (3, 32, 32)
    ...
    model = PixelCNN(nr_resnet=config.nr_resnet, nr_filters=config.nr_filters,
                     input_channels=3, nr_logistic_mix=config.nr_logistic_mix).cuda()
